...
class articleModel{
	...
	public function getJoinList($condition,$page=''){
		...
		$condition_str	= $this->_condition($condition);
		$param	= array();
		$param['table'] = empty($condition['table'])?'article,article_class,upload':$condition['table'];
		$param['field']	= empty($condition['field'])?'*':$condition['field'];;
		$param['join_type']	= empty($condition['join_type'])?'left join':$condition['join_type'];
		$param['join_on']	=empty($condition['join_on']) ?array('article.ac_id=article_class.ac_id','article.article_id=upload.item_id'):$condition['join_on'];
		$param['where'] = $condition_str;
		$param['limit'] = $condition['limit'];
