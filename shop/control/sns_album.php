...
class sns_albumControl extends BaseSNSControl {
	...
	public function album_addOp() {
		 ...
		 */
		$model = Model ();
		$class_count = $model->table ( 'sns_albumclass' )->where ( array (
	...
	public function album_editOp() {
		$id = intval ( $_GET ['id'] );
		if ($id <= 0) {
			echo Language::get ( 'album_parameter_error' );
			exit ();
		}
		/**
		 * 实例化相册模型
	...
	public function album_delOp() {
		 ...
		 * 删除分类
		 */
		$return = $model->table ( 'sns_albumclass' )->where ( array (
				'ac_id' => $id,
				'member_id' => $_SESSION ['member_id'] 
		) )->delete ();
		if (! $return) {
	...
	public function album_pic_delOp() {
		...
		foreach ( $id as $v ) {
			...
			if ($v <= 0)
				continue;
			$ap_info = $model->table ( 'sns_albumpic' )->where ( array (
					'ap_id' => $v,
					'member_id' => $_SESSION ['member_id'] 
			) )->find ();
			if (empty ( $ap_info ))
		...
		switch ($menu_type) {
			case 'album' :
				$menu_array = array (
						1 => array (
								'menu_key' => 'album',
								'menu_name' => $album_menuname,
								'menu_url' => 'index.php?act=sns_album&mid=' . $this->master_id 
						) 
				);
				break;
			case 'album_pic' :
				$menu_array = array (
						...
						2 => array (
								'menu_key' => 'pic_list',
								'menu_name' => Language::get ( 'nc_sns_path_album_pic_list' ),
								'menu_url' => 'index.php?act=sns_album&op=album_pic_list&id=' . intval ( $_GET ['id'] ) . '&mid=' . $this->master_id 
						) 
				);
				break;
			case 'album_pic_info' :
				$menu_array = array (
						...
						2 => array (
								'menu_key' => 'pic_info',
								'menu_name' => Language::get ( 'nc_sns_path_album_pic_info' ),
								'menu_url' => 'index.php?act=sns_album&op=album_pic_info&id=' . intval ( $_GET ['id'] ) . '&class_id=' . intval ( $_GET ['class_id'] ) . '$mid=' . $this->master_id 
						) 
				);
				break;
		}
	...
	public function swfuploadOp() {
		...
		if ($member_id <= 0 && $class_id <= 0) {
			echo json_encode ( array (
					'state' => 'false',
					'message' => Language::get ( 'sns_upload_pic_fail' ),
		...
		$model = Model ();
		// 验证图片数量
		$count = $model->table ( 'sns_albumpic' )->where ( array (
				'member_id' => $member_id 
		) )->count ();
		if (C ( 'malbum_max_sum' ) != 0 && $count >= C ( 'malbum_max_sum' )) {
			echo json_encode ( array (
					'state' => 'false',
					'message' => Language::get ( 'sns_upload_img_max_num_error' ),
					'origin_file_name' => $_FILES ["file"] ["name"] 
			) );
			exit ();
		}
