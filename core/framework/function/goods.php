...
function snsThumb($image_name = '', $type = '') {
	...
	//缩略图路径
	$thumb_file_path = ATTACH_MALBUM . DS . str_ireplace ( '.', '_' . $type . '.', $image_name );
	//原图路径
	$file_path = ATTACH_MALBUM . DS . $image_name;
	
	if(file_exists ( BASE_UPLOAD_PATH . DS . $thumb_file_path )){
		return UPLOAD_SITE_URL . DS . $thumb_file_path;
	}elseif(file_exists ( BASE_UPLOAD_PATH . DS . $file_path)){
		return UPLOAD_SITE_URL . DS . $file_path;
	}
