...
<div class="page">
  <div class="fixed-bar">
    <div class="item-title">
      ...
      <ul class="tab-base">
        <li><a href="JavaScript:void(0);" class="current"><span><?php echo '集团首页banner';?></span></a></li>
      </ul>
    </div>
  </div>
  ...
  <div class="homepage-focus" id="homepageFocusTab">
    ...
    <form id="upload_screen_form" class="tab-content" name="upload_screen_form" enctype="multipart/form-data" method="post" action="index.php?act=web_api&op=screen_pic" target="upload_pic">
      ...
      <div class="full-screen-slides">
        ...
        <div class="add-focus"><a class="btn-add-nofloat" href="JavaScript:add_video_screen('pic');"><?php echo '图片调用';?></a>
            ...
            <?php } ?>
            <span class="s-tips"><i></i><?php echo '小提示：单击图片选中修改，拖动可以排序，添加最多不超过2个，保存后生效。';?></span></div>
      </div>
    ...
    <!-- 加入我们展示 start -->
    <form id="upload_join_form" class="tab-content" name="upload_screen_form" enctype="multipart/form-data" method="post" action="index.php?act=web_api&op=screen_pic&flag=2" target="upload_pic" style="display:none;">
      <input type="hidden" name="form_submit" value="ok" />
      <input type="hidden" name="web_id" value="151">
      <input type="hidden" name="code_id" value="<?php echo $output['code_index_screen_list']['code_id'];?>">
      <input type="hidden" name="key" value="">
      <div class="full-screen-slides">
        <ul>
          ...
          <li ap="1" screen_id="<?php echo $val['pic_id'];?>" title="可上下拖拽更改显示顺序">
            ...
            <div class="focus-thumb" onclick="select_screen(<?php echo $val['pic_id'];?>);" style="background-color:<?php echo $val['color'];?>;" title="点击编辑选中区域内容">
                <img title="<?php echo $val['pic_name'];?>" src="<?php echo UPLOAD_SITE_URL.'/'.$val['pic_img'];?>"/></div>
            <input name="index_screen_list[<?php echo $val['pic_id'];?>][pic_id]" value="<?php echo $val['pic_id'];?>" type="hidden">
            <input name="index_screen_list[<?php echo $val['pic_id'];?>][ap_id]" value="<?php echo $val['ap_id'];?>" type="hidden">
            <input name="index_screen_list[<?php echo $val['pic_id'];?>][pic_name]" value="<?php echo $val['pic_name'];?>" type="hidden">
            <input name="index_screen_list[<?php echo $val['pic_id'];?>][color]" value="<?php echo $val['color'];?>" type="hidden">
            <input name="index_screen_list[<?php echo $val['pic_id'];?>][pic_img]" value="<?php echo $val['pic_img'];?>" type="hidden">
          </li>
          <?php }else { ?>
          <li ap="0" screen_id="<?php echo $val['pic_id'];?>" title="可上下拖拽更改显示顺序">
            图片调用
                <a class="del" href="JavaScript:del_group_screen(<?php echo $val['pic_id'];?>);" title="<?php echo $lang['nc_del'];?>">X</a>
            <div class="focus-thumb" onclick="select_group_screen(<?php echo $val['pic_id'];?>);" style="background-color:<?php echo $val['color'];?>;" title="点击编辑选中区域内容">
                <img title="<?php echo $val['pic_name'];?>" src="<?php echo UPLOAD_SITE_URL.'/'.$val['pic_img'];?>"/></div>
            <input name="index_screen_list[<?php echo $val['pic_id'];?>][pic_id]" value="<?php echo $val['pic_id'];?>" type="hidden">
            <input name="index_screen_list[<?php echo $val['pic_id'];?>][pic_name]" value="<?php echo $val['pic_name'];?>" type="hidden">
            <input name="index_screen_list[<?php echo $val['pic_id'];?>][pic_url]" value="<?php echo $val['pic_url'];?>" type="hidden">
            <input name="index_screen_list[<?php echo $val['pic_id'];?>][color]" value="<?php echo $val['color'];?>" type="hidden">
            <input name="index_screen_list[<?php echo $val['pic_id'];?>][pic_img]" value="<?php echo $val['pic_img'];?>" type="hidden">
          </li>
          <?php } ?>
        ...
        <div class="add-focus"><a class="btn-add-nofloat" href="JavaScript:add_group_screen('pic');"><?php echo '图片调用';?></a>
            <?php if(!empty($output['screen_adv_list']) && is_array($output['screen_adv_list'])){ ?>
            <a class="btn-add-nofloat" href="JavaScript:add_screen('adv');"><?php echo '广告调用';?></a>
            <?php } ?>
            <span class="s-tips"><i></i><?php echo '小提示：单击图片选中修改，拖动可以排序，添加最多不超过2个，保存后生效。';?></span></div>
      </div>
      <table id="ap_screen_group" class="table tb-type2" style="display:none;">
        <tbody>
          <tr>
            <td colspan="2" class="required"><?php echo '广告位'.$lang['nc_colon'];?></td>
          </tr>
      ...
      <table id="upload_screen_group" class="table tb-type2" style="display:none;">
        <tbody>
          <tr>
            <td colspan="2" class="required"><?php echo '文字标题'.$lang['nc_colon'];?></td>
          </tr>
          ...
          <tr class="noborder">
            ...
            <td class="vatop tips">为确保显示效果正确，请选择W:701px H:381px的清晰图片作为全屏焦点图。</td>
          </tr>
          <tr>
            <td colspan="2" class="required"><label><?php echo '背景颜色'.$lang['nc_colon'];?></label></td>
          </tr>
      ...
      <div class="margintop"><a href="JavaScript:void(0);" onclick="$('#upload_join_form').submit();" class="btn"><span><?php echo $lang['web_config_save'];?></span></a>
        <a href="index.php?act=web_api&op=html_update&web_id=<?php echo $output['code_screen_list']['web_id'];?>" class="btn"><span><?php echo $lang['web_config_web_html'];?></span></a>
        <span class="web-save-succ" style="display:none;"><?php echo $lang['nc_common_save_succ'];?></span>
        </div>
    </form>
