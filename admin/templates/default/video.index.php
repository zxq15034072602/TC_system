...
<div class="page">
    ...
    <table class="tb-type1 noborder search">
      <tbody>
        <tr>
          ...
          <th><label for="search_ac_id"><?php echo $lang['video_index_class'];?></label></th>
          <td><select name="search_vd_id" id="search_vd_id" class="">
              <option value=""><?php echo $lang['nc_please_choose'];?>...</option>
              <?php if(!empty($output['parent_list']) && is_array($output['parent_list'])){ ?>
              <?php foreach($output['parent_list'] as $k => $v){ ?>
              <option <?php if($output['search_vd_id'] == $v['vd_id']){ ?>selected='selected'<?php } ?> value="<?php echo $v['vd_id'];?>"><?php echo $v['vd_name'];?></option>
              <?php } ?>
              <?php } ?>
            </select></td>
          <td><a href="javascript:document.formSearch.submit();" class="btn-search " title="<?php echo $lang['nc_query'];?>">&nbsp;</a>
            <?php if($output['search_title'] != '' or $output['search_vd_id'] != ''){?>
            <a href="index.php?act=video&op=video" class="btns " title="<?php echo $lang['nc_cancel_search'];?>"><span><?php echo $lang['nc_cancel_search'];?></span></a>
            <?php }?></td>
        </tr>
      </tbody>
    </table>
  ...
  <table class="table tb-type2" id="prompt">
    <tbody>
      ...
      <tr>
        <td><ul>
            <li><?php echo $lang['video_index_help1'];?></li>
          </ul></td>
      </tr>
    </tbody>
  </table>
    ...
    <table class="table tb-type2">
      ...
      <tbody>
        ...
        <tr class="hover">
          ...
          <td class="align-center"><a href="index.php?act=video&op=video_edit&video_id=<?php echo $v['video_id']; ?>"><?php echo $lang['nc_edit'];?></a></td>
        </tr>
        <?php } ?>
        <?php }else { ?>
        <tr class="no_data">
