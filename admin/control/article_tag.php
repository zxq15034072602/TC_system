...
class article_tagControl extends SystemControl{
	...
	public function indexOp() {
        $this->article_tag_listOp();
	}

    /**
     * 文章标签列表
     **/
    public function article_tag_listOp() {
        $model = Model('article_tag');
        $list = $model->getList(TRUE, null, 'tag_id desc');
        $this->show_menu('list');
        Tpl::output('list',$list);
        Tpl::showpage("article_tag.list");
    }

    /**
     * cms文章分类添加
     **/
    public function article_tag_addOp() {
        $this->show_menu('add');
        Tpl::showpage('article_tag.add');
    }

    /**
     * cms文章分类保存
     **/
    public function article_tag_saveOp() {
        ...
        if($result) {
            ...
            showMessage(Language::get('tag_add_success'),'index.php?act=article_tag&op=article_tag_list');
        } else {
            $this->log(Language::get('cms_log_tag_save').$result, 0);
            showMessage(Language::get('tag_add_fail'),'index.php?act=article_tag&op=article_tag_list','','error');
        }
     ...
     * cms标签删除
     **/
     public function article_tag_dropOp() {
    ...
    private function show_menu($menu_key) {
        $menu_array = array(
            'list'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_list'),'menu_url'=>'index.php?act=article_tag&op=article_tag_list'),
            'add'=>array('menu_type'=>'link','menu_name'=>Language::get('nc_new'),'menu_url'=>'index.php?act=article_tag&op=article_tag_add'),
        );
