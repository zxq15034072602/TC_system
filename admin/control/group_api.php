...
class group_apiControl extends SystemControl {
    ...
    public function focus_editOp() {
        ...
        $code_list = $model_web_config->getCodeList(array('web_id'=> 151));
        if(is_array($code_list) && !empty($code_list)) {
            foreach ($code_list as $key => $val) {//将变量输出到页面
                $var_name = $val['var_name'];
                $code_info = $val['code_info'];
        ...
        $screen_adv_list = $model_web_config->getAdvList("screen");//集团首页加入我们展示
        Tpl::output('index_screen_adv_list',$screen_adv_list);
    
        Tpl::showpage('group_focus.edit');
    }
    /**
     * 保存焦点区切换小图
     */
